                ...
                try:
                    with zipfile.ZipFile(f, 'r') as zf:
                        for file_info in zf.filelist:
                            if file_info.filename.endswith(('.java', '.xml', '.gradle', '.kt', '.py', '.js', '.ts')):
                                try:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: ai-zip.py 'your prompt' [file1] [file2] ... [project.zip]")
        sys.exit(1)
    
    prompt = sys.argv[1]
    files = sys.argv[2:] if len(sys.argv) > 2 els